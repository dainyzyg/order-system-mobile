...
.page
    ...
    .card-content(ref="content" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="100")
      .card(v-for="item,i in datalist" :key="item.number" @click="showDetail(item,i)")
        .card-header
          ...
          .date {{!item.orderDate?'':`订单日期: ${item.orderDate}`}}
        .card-body
  ...
  SearchPanel(dateName="订单日期" roleValue="subscriber" :show.sync="searchPanelVisible" @search="search")
  Detail(store="order" storeName="订单" :show.sync="detailVisible" :value.sync="currentRecord" @addcomplete="initData" @savecomplete="modifyRecord")
</template>
...
export default {
  ...
  methods: {
    ...
    async search(val) {
      ...
      let datalist
      if (keyRangeValue) {
        datalist = await this.$IDB.executeCursor('order', 'orderDate', keyRangeValue, cursor => {
          cursor.continue()
        ...
        datalist = await this.$IDB.executeCursor('order', null, null, cursor => {
          cursor.continue()
          const subscriberReg = new RegExp(val.subscriber, 'i')
          const modelReg = new RegExp(val.model, 'i')
          const subscriberTest = !val.subscriber || subscriberReg.test(cursor.value.subscriber)
    ...
    async orderchange(index) {
      ...
      if (index === 'default') {
        ...
        this.recordList = await this.$IDB.openIndexCursor('order', index)
        this.datalist = []
        for (let i = 0; i <= 10; i++) {
          if (i < this.recordList.length) {
            this.datalist.push(this.recordList[i])
    ...
    async initData() {
      this.recordList = await this.$IDB.openCursor('order')
      this.datalist = []
      for (let i = 0; i <= 10; i++) {
        if (i < this.recordList.length) {
          this.datalist.push(this.recordList[i])
