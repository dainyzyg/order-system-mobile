...
.page
    ...
    .card-content(ref="content" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="100")
      .card(v-for="item,i in datalist" :key="item.number" @click="showDetail(item,i)")
        .card-header
          ...
          .date {{!item.saleDate?'':`销售日期: ${item.saleDate}`}}
        .card-body
  ...
  FallingModal(roleName="购买人" roleValue="purchaser" :show.sync="popupVisible" @orderchange="orderchange")
  SearchPanel(dateName="销售日期" roleValue="purchaser" :show.sync="searchPanelVisible" @search="search")
  Detail(store="sales" storeName="销售" :show.sync="detailVisible" :value.sync="currentRecord" @addcomplete="initData" @savecomplete="modifyRecord")
</template>
...
export default {
  ...
  methods: {
    ...
    async search(val) {
      ...
      let datalist
      if (keyRangeValue) {
        datalist = await this.$IDB.executeCursor('sales', 'saleDate', keyRangeValue, cursor => {
          cursor.continue()
          const purchaserReg = new RegExp(val.purchaser, 'i')
          const modelReg = new RegExp(val.model, 'i')
          const purchaserTest = !val.purchaser || purchaserReg.test(cursor.value.purchaser)
          ...
          return purchaserTest && modelTest && sizeTest
        })
      } else {
        datalist = await this.$IDB.executeCursor('sales', null, null, cursor => {
          cursor.continue()
          const purchaserReg = new RegExp(val.purchaser, 'i')
          const modelReg = new RegExp(val.model, 'i')
          const purchaserTest = !val.purchaser || purchaserReg.test(cursor.value.purchaser)
    ...
    async orderchange(index) {
      ...
      if (index === 'default') {
        ...
        this.recordList = await this.$IDB.openIndexCursor('sales', index)
        this.datalist = []
        for (let i = 0; i <= 10; i++) {
          if (i < this.recordList.length) {
            this.datalist.push(this.recordList[i])
    ...
    async initData() {
      this.recordList = await this.$IDB.openCursor('sales')
      this.datalist = []
      for (let i = 0; i <= 10; i++) {
        if (i < this.recordList.length) {
          this.datalist.push(this.recordList[i])
